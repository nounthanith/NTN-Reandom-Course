    ...
    <div className="w-full pt-20 px-4 sm:px-6 lg:px-8 min-h-screen bg-base-100">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Featured Courses</h1>
            <p className="mt-2 text-gray-600 dark:text-gray-300">Browse and manage all available courses</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-0 mb-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="md:col-span-2">
              <label className="label font-medium text-sm text-base-content">Search Courses</label>
              <input
                type="text"
                name="searchQuery"
            ...
            <div className="md:col-span-2 flex flex-col sm:flex-row gap-4">
              <div>
                <label className="label font-medium text-sm text-base-content">Filter by Category</label>
                <select
                  name="category"
              ...
              <div>
                <label className="label font-medium text-sm text-base-content">Filter by Language</label>
                <select
                  name="language"
                  className="select select-bordered select-sm w-full"
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              {filters.category && (
                <div className="badge badge-primary gap-2">
                  ...
                  <button onClick={() => setFilters((prev) => ({ ...prev, category: "" }))} className="ml-1 text-white">×</button>
                </div>
              )}
              {filters.language && (
                <div className="badge badge-secondary gap-2">
                  Language: {filters.language}
                  <button onClick={() => setFilters((prev) => ({ ...prev, language: "" }))} className="ml-1 text-white">×</button>
                </div>
              )}
              {filters.searchQuery && (
                <div className="badge badge-accent gap-2">
                  Search: {filters.searchQuery}
                  <button onClick={() => setFilters((prev) => ({ ...prev, searchQuery: "" }))} className="ml-1 text-white">×</button>
                </div>
              )}
              <button onClick={clearFilters} className="btn btn-xs btn-outline btn-error ml-2">Clear all</button>
            </div>
          )}
        </div>
            ...
            <div className="max-w-md text-center">
              <div className="card bg-base-200 shadow-xl">
                ...
                <div className="card-body">
                  <h2 className="card-title justify-center">No Courses Found</h2>
        ...
        {!isLoading && currentCourses.length > 0 && (
          <>
            <div className="mb-4 text-sm text-gray-500">
              Showing {currentCourses.length} of {filteredCourses.length} filtered courses
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
              {currentCourses.map((course) => (
                <div key={course[0]} className="card bg-base-200 shadow-sm hover:shadow-md transition-shadow duration-300 h-full flex flex-col">
                  ...
                  <div className="card-body flex-1">
                    <h2 className="card-title text-lg line-clamp-2">{course[1]}</h2>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-3 flex-1">
                      {course[3] && course[3] !== "None" ? course[3] : "No description provided."}
                    </p>
                    <div className="mt-4 flex justify-between items-center">
                      <a href={course[4]} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-primary">
                        View Course
                      </a>
